...
public class DataBaseServiceImpl extends SuperServiceImpl<DataBaseMapper, DataBase> implements DataBaseService {
    ...
    @Override
    public void aiDIMModel(AiDIMModel aiDIMModel) {
        DataBase database = getById(aiDIMModel.getId());
        Asserts.checkNotNull(database,"该数据源不存在");
        APIExecuteSqlDTO apiExecuteSqlDTO = new APIExecuteSqlDTO();
        apiService.executeSql(apiExecuteSqlDTO);
//        Driver driver = Driver.build(dataBase.getDriverConfig());
    }
